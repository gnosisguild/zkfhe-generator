//! Config file generation for BFV Decryption circuit (no homomorphic addition)
//!
//! This module generates a .nr config file with all circuit-specific configs
//! (N, H, L, L_PRIME, QIS, bounds, bit parameters, Configs) that can be imported in the main circuit.

use crate::bounds::{DecBfvNoHomAddBounds, DecBfvNoHomAddCryptographicParameters};
use crate::template::DecBfvNoHomAddTemplateParams;
use shared::errors::ZkFheResult;
use std::path::{Path, PathBuf};

/// Generator for BFV Decryption circuit (no homomorphic addition) config files
pub struct DecBfvNoHomAddConfigsGenerator;

impl DecBfvNoHomAddConfigsGenerator {
    ...
    /// * `template_params` - Template parameters (N, H, L, L_PRIME, bit widths)
    ...
    pub fn generate_configs(
        ...
        template_params: &DecBfvNoHomAddTemplateParams,
        _parameter_type: &str,
    ) -> ZkFheResult<String> {
        ...
        let configs = format!(
            r#"use crate::core::bfv_dec_no_hom_add::Config as BfvDecNoHomAddConfig;

// Global configs for BFV Decryption circuit (no homomorphic addition)
pub global N: u32 = {};
...
"#,
            ...
            template_params.num_trbfv_bases,     // L
            ...
            crypto_params.bfv_plaintext_modulus, // BFV_PLAINTEXT_MODULUS
            crypto_params.bfv_q_inverse_mod_t,   // BFV_Q_INVERSE_MOD_T
