...
/// * `mode` - Encryption mode (Key or Share)
///
/// # Mode Behavior
///
/// * `Key` - Generates sample message/vote encryption (default behavior)
/// * `Share` - Generates sample threshold secret key share encryption
pub fn generate_sample_encryption(
    ...
    // Generate keys (same for both modes)
    let sk = SecretKey::random(params, &mut rng);
    let pk = PublicKey::new(&sk, &mut rng);

    let pt = match mode {
        GrecoMode::Key => {
            // Original: encrypt a message/vote
        ...
        GrecoMode::Share => {
            ...
            // Here we are using a simple example with 3 parties and a threshold of 1 since these
