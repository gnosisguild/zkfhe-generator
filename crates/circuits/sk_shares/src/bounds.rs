...
impl SkSharesBounds {
    /// Computes the cryptographic parameters and numeric bounds from BFV parameters.
    ///
    /// Returns `(crypto_params, bounds)` where:
    /// - `crypto_params.qis` is copied from BFV moduli.
    /// - `randomness_bound = max_j floor((q_j - 1) / 2)`.
    /// - `(r_lower_bound, r_upper_bound)` is derived by `r_bounds(n, t, moduli)`.
    /// - `sk_bound` is set to `1`.
    pub fn compute(
        bfv_params: &Arc<BfvParameters>,
    ) -> ZkFheResult<(SkSharesCryptographicParameters, Self)> {
        let ctx = bfv_params.ctx_at_level(0)?;
        let moduli: Vec<u64> = ctx.moduli().to_vec();
        ...
        let (r_lower_bound, r_upper_bound) = r_bounds(n, t as usize, &moduli);
        let sk_bound = 1;
