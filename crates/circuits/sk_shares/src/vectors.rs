...
impl SkSharesVectors {
    ...
    pub fn compute(
        ...
        for i in 0..degree {
            ...
            for (j, &qj_u) in moduli.iter().enumerate() {
                ...
                f[i][j] = sss.sample_polynomial(a_i.clone(), &mut rng);
    ...
    pub fn standard_form(&self) -> Self {
        ...
        SkSharesVectors {
            ...
            f: reduce_coefficients_3d(&self.f, zkp_modulus),
            y: reduce_coefficients_3d(&self.y, zkp_modulus),
            r: reduce_coefficients_3d(&self.r, zkp_modulus),
            d: reduce_coefficients_2d(&self.d, zkp_modulus),
            f_randomness: reduce_coefficients_2d(&self.f_randomness, zkp_modulus),
            x_coords: reduce_coefficients(&self.x_coords, zkp_modulus),
        }
    }
}
